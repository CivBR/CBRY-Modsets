					...
					if (mPlayer:IsAlive()) then			
						...
						if Teams[mTeam]:IsAtWar(pTeam) then
							...
							elseif BlockadeCount <= 3 then
								PirateGPT = (GPTBonus * ((BlockadeCount * 2) + 1)); -- Steal 10% GPT, +20% per Blockade
							end

							local mReserves = (mPlayer:GetGold());

							local ReservesPirate = 0;
							if BlockadeCount >= 4 then  
								ReservesPirate = ((8 * mReserves) / 100); -- Cap at 8% 
							elseif BlockadeCount <= 3 then
								ReservesPirate = (((1 + (BlockadeCount * 2)) * mReserves) / 100); -- Steal 1% Gold Reserves, +2% per Blockade
							end		
							...
							if BlockadeCount == 0 then			
								if PirateCount >= 15 then
									PirateCount = 15;
								end
							elseif BlockadeCount == 1 then
								if PirateCount >= 30 then
									PirateCount = 30;
								end
							elseif BlockadeCount == 2 then
								if PirateCount >= 45 then
									PirateCount = 45;
								end
							elseif BlockadeCount == 3 then
								if PirateCount >= 60 then
									PirateCount = 60;
								end
							elseif BlockadeCount >= 4 then
								if PirateCount >= 75 then
									PirateCount = 75;
								end
							end
							--
							...
							local aiReservesPirate = ((2 * mReserves) / 100);
							local aiPirateBonus = math.ceil(((GPTBonus * 2) + aiReservesPirate));
							if aiPirateBonus >= 15 then
								aiPirateBonus = 15;
							end
							if not (pPlayer:IsHuman()) and (Teams[mTeam]:GetCurrentEra() > eRen) then
