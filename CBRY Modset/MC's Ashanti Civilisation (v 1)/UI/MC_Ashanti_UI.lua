...
function MC_Ashanti_AI_Support(iPlayer)
	...
	local iUnit = GetStrongestMilitaryUnit(pPlayer, false, "UNITCOMBAT_MELEE", "UNITCOMBAT_GUN", "UNITCOMBAT_ARCHER")
	...
	if not(iGSP < iCost) then
		local pUnit = InitUnitFromCity(pCity, tUnit.ID, 1)[1]
		LuaEvents.MC_GoldenStoolUnitPurchased(pPlayer:GetID(), pCity:GetID(), pUnit:GetID())
		save(pPlayer, "GSP", (iGSP-iCost))
	end
end
