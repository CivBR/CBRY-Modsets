...
function ShowInPediaShow(table, data)
	if (gShowInPediaShowSupport[table] == nil) then
		gShowInPediaShowSupport[table] = false
		...
		for row in DB.Query("SELECT * FROM " .. table .. " LIMIT 1") do
			for col, _ in pairs(row) do
				if (col == "ShowInPedia") then
					gShowInPediaShowSupport[table] = true
					break
				end
			end
		end
	...
	if (gShowInPediaShowSupport[table] == false) then
		-- The default is to show all items in the Civilopedia for tables that don't have a ShowInPedia column
		return true
	end

	-- As the ShowInPedia columns are added by altering the table with SQL, the game core doesn't know about them
	-- so we can't use data.ShowInPedia, hence we need to execute a DB.Query() to get the value directly and check it
	for row in DB.Query("SELECT ShowInPedia FROM " .. table .. " WHERE ID=" .. data.ID) do
		-- Explicit check for ShowInPedia required, as in Lua the number 0 is also "true"
		return (row.ShowInPedia == 1 or row.ShowInPedia == true)
	end
...
--			local civ = GameInfo.Civilizations[civilizationType];
			local civ = ShowInPediaLookup("Civilizations", civilizationType);
			if(civ ~= nil) then
